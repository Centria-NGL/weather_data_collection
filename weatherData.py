...
def reqWeatherData():
    rawData = dict()
    try:
        rawData = client.getForecastWeather(q='New York', days = 4)
...
def forecastWeatherData_3_days(rawData):
        ...
        try:
                ...
                for d in day:
                    data[d['date']] = list()
                    time = datetime.datetime.now(tz.gettz("America/New_York")).strftime("%H:%M:%S")
                    data[d['date']] = [d['date'], time,
                    d['avgtemp_c'], d['avghumidity'],
                    d['maxwind_kph']]

                    percip = d['totalprecip_mm']
...
def forecastWeatherData_3rd_Day(rawData):
    ...
    try:
            ...
            data[date] = [date, time,
            day['day']['avgtemp_c'], day['day']['avghumidity'],
            day['day']['maxwind_kph']]
