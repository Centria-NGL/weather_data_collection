...
REQ_INTERVALS = 3600
logging.basicConfig(filename= 'log_WD_process.log', level = logging.DEBUG,
...
set_current_zero_command_sql = ("UPDATE WEATHER_CURRENT "
                               "SET IS_CURRENT = ? "
                               " WHERE LAST_UPDATED_TIME = ?")

retrieve_last_updated_time = "SELECT TOP 1 * FROM WEATHER_CURRENT ORDER BY ROWID DESC"
